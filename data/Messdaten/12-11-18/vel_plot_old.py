...
r = 6371000.785	
for data in dataImu:
	x_data.append( data.angular_velocity.x )
	y_data.append( data.angular_velocity.y )
	z_data.append( data.angular_velocity.z )



def sum_integrate(data, point):
	#print(integrate.simps(data[:point]))
	return integrate.simps(data[:point])
...
ax.plot(x_integrated, y_integrated, z_integrated, label='copter')
ax.legend()
